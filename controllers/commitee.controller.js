...
function read(req, res) {
  connect.con.query("SELECT id, nome, email FROM commitee ", function (
    err,
    rows,
    fields
  ) {
...
function readID(req, res) {
  const idcommitee = req.sanitize("id").escape();
  const post = { idSponsor: idcommitee };
  connect.con.query(
    "SELECT id, nome, email FROM commitee where id = ?",
    post,
...
function update(req, res) {
  ...
  req.checkBody("nome", "Insira apenas texto").matches(/^[a-z ]+$/i);
  req.checkBody("email", "Insira um email válido.").isEmail();
  req.checkParams("id", "Insira um ID de commitee válido").isNumeric();
  const errors = req.validationErrors();
  if (errors) {
    res.send(errors);
    return;
    ...
    if (
      ...
      typeof idcommitee != "undefined"
    ) {
      const update = [nome, email, idcommitee];
      const query = connect.con.query(
        "UPDATE commitee SET nome =?, email =? WHERE id=?",
        update,
        function (err, rows, fields) {
          console.log(query.sql);
          if (!err) {
...
function deleteL(req, res) {
  ...
  const query = connect.con.query(
    "UPDATE commitee SET active = ? WHERE id=?",
    update,
    function (err, rows, fields) {
      console.log(query.sql);
      if (!err) {
